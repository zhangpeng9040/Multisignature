...
func testCollectiveSignandVerify(t *testing.T, c elliptic.Curve, tag string, timetemp1, timetemp2, timetemp3 *int64) {
	var number int
	number = 32
	var priv []*PrivateKey
	var rcomit1 []*big.Int
	...
	*timetemp1 += time2 - timeoff;
	*timetemp2 += time3 - time2
	*timetemp3 += timeoff - time1
...
func TestCollectiveSignVerify(t *testing.T) {
	var timetemp1, timetemp2, timetemp3 int64
	for i :=0; i < 1; i ++ {
		testCollectiveSignandVerify(t, elliptic.P256(), "p256", &timetemp1, &timetemp2, &timetemp3)
	}
	fmt.Println("online sign:",timetemp1)
	fmt.Println("offline sign:", timetemp3)
